...
from reco_encoder.model import model_original as model_orig
...
def log_var_and_grad_summaries(logger, layers, global_step, prefix, log_histograms=False):
  ...
  for ind, w in enumerate(layers):
    ...
    logger.add_scalar("Variables/FrobNorm/{}_{}".format(prefix, ind), np.linalg.norm(w_var),
                          global_step)
    if log_histograms:
      logger.add_histogram(tag="Variables/{}_{}".format(prefix, ind), values=w.data.cpu().numpy(),
                           global_step=global_step)

    # Gradients
    w_grad = w.grad.data.cpu().numpy()
    logger.add_scalar("Gradients/FrobNorm/{}_{}".format(prefix, ind), np.linalg.norm(w_grad),
                          global_step)
    if log_histograms:
      logger.add_histogram(tag="Gradients/{}_{}".format(prefix, ind), values=w.grad.data.cpu().numpy(),
                         global_step=global_step)
...
def main():
  ...
  for epoch in range(args.num_epochs):
    ...
    for i, mb in enumerate(data_layer.iterate_one_epoch()):
      ...
      if i % args.summary_frequency == 0:
        ...
        logger.add_scalar("Training_RMSE", sqrt(t_loss/t_loss_denom), global_step)
        t_loss = 0
        t_loss_denom = 0.0
        log_var_and_grad_summaries(logger, rencoder.encode_w, global_step, "Encode_W", True)
        log_var_and_grad_summaries(logger, rencoder.encode_b, global_step, "Encode_b", True)
        if not rencoder.is_constrained:
          log_var_and_grad_summaries(logger, rencoder.decode_w, global_step, "Decode_W", True)
        log_var_and_grad_summaries(logger, rencoder.decode_b, global_step, "Decode_b", True)
    ...
    logger.add_scalar("Training_RMSE_per_epoch", sqrt(total_epoch_loss/denom), epoch)
    logger.add_scalar("Epoch_time", e_end_time - e_start_time, epoch)
    ...
    if epoch % 3 == 0 or epoch == args.num_epochs - 1:
      ...
      logger.add_scalar("EVALUATION_RMSE", eval_loss, epoch)
  ...
  ###############################################################
  ###################### 원래 모델과 비교!! #######################
  rencoder = model_orig.AutoEncoder(
      layer_sizes=[data_layer.vector_dim] + [int(l) for l in args.hidden_layers.split(',')],
      nl_type=args.non_linearity_type,
      is_constrained=args.constrained,
      dp_drop_prob=args.drop_prob,
      last_layer_activations=not args.skip_last_layer_nl)
  ...
  for epoch in range(args.num_epochs):
      ...
      for i, mb in enumerate(data_layer.iterate_one_epoch()):
          ...
          if i % args.summary_frequency == 0:
              print('[%d, %5d] RMSE: %.7f' % (epoch, i, sqrt(t_loss / t_loss_denom)))
              logger.add_scalar("ORIG_Training_RMSE", sqrt(t_loss / t_loss_denom), global_step)
              t_loss = 0
              t_loss_denom = 0.0
              log_var_and_grad_summaries(logger, rencoder.encode_w, global_step, "ORIG_Encode_W", True)
              log_var_and_grad_summaries(logger, rencoder.encode_b, global_step, "ORIG_Encode_b", True)
              if not rencoder.is_constrained:
                  log_var_and_grad_summaries(logger, rencoder.decode_w, global_step, "ORIG_Decode_W", True)
              log_var_and_grad_summaries(logger, rencoder.decode_b, global_step, "ORIG_Decode_b", True)
      ...
      print('Total epoch {} finished in {} seconds with TRAINING RMSE loss: {}'
            .format(epoch, e_end_time - e_start_time, sqrt(total_epoch_loss / denom)))
      logger.add_scalar("ORIG_Training_RMSE_per_epoch", sqrt(total_epoch_loss / denom), epoch)
      logger.add_scalar("ORIG_Epoch_time", e_end_time - e_start_time, epoch)

      if epoch % 3 == 0 or epoch == args.num_epochs - 1:
          eval_loss = do_eval(rencoder, eval_data_layer)
          print('Epoch {} EVALUATION LOSS: {}'.format(epoch, eval_loss))
          logger.add_scalar("ORIG_EVALUATION_RMSE", eval_loss, epoch)
