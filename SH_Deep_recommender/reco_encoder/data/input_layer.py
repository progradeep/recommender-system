...
class UserItemRecDataProvider:
  ...
  def iterate_one_epoch(self):
    ...
    shuffle(keys)
    s_ind = 0
    e_ind = self._batch_size # 128
    while e_ind < len(keys): # key 개수보다 batch size가 작을 동안:
      local_ind = 0
      inds1 = []
      inds2 = []
      vals = []
      for ind in range(s_ind, e_ind):
        inds2 += [v[0] for v in data[keys[ind]]] # itemId 추가
        inds1 += [local_ind]*len([v[0] for v in data[keys[ind]]]) # itemID 만큼 0 생성, 추가
        vals += [v[1] for v in data[keys[ind]]] # rating 추가
        local_ind += 1

      i_torch = torch.LongTensor([inds1, inds2]) # (2, inds2 + inds1)
      # print("i torchshape",i_torch.shape)
      v_torch = torch.FloatTensor(vals)
      # print("v torchshape", v_torch.shape)
      ...
      # print("minibatch shape", mini_batch.shape)
